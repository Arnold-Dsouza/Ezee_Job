...
5. LATEX ESCAPING: Ensure all LaTeX special characters in textual fields are properly escaped:
   ...
   - Escape _ as \_
   - Escape { as \{ and } as \}
6. COVER LETTER HEADING RULE: You MUST include "Cover Letter" as a bold centered heading (using \\begin{center}\\textbf{\\Large Cover Letter}\\end{center}) above the recipient address block for all Cover Letter generations.
7. COVER LETTER ADDRESS FORMAT: Do not place the candidate's personal information at the top of the cover letter. Start the body with a recipient block beginning with "To," and then the person or hiring team the letter is addressed to. If the recipient is known from the job description, include their name and organization; otherwise use a professional generic recipient block.
8. COVER LETTER SUBJECT LINE: Include the subject line directly before the salutation (Dear Hiring Manager), not as "Re:". Format: \\textbf{Application for [Target Role]}.
9. JSON OUTPUT ONLY: You MUST return a single valid JSON object.
...
- "coverLetterHtml": Complete, self-contained HTML string for the Cover Letter (must include a bold centered heading that reads "Cover Letter").
...
<head>
  <meta charset="utf-8">
  <style>
    body { font-family: Arial, sans-serif; color: #111827; margin: 0; padding: 48px; line-height: 1.55; }
...
function convertFallbackCoverLetterToHtml(source) {
  ...
  const plain = source
    ...
    .replace(/\\begin\{center\}[\s\S]*?Cover Letter[\s\S]*?\\end\{center\}/i, '')
    .replace(/\\vspace\{[^}]*\}/g, '')
    .replace(/\\noindent/g, '')
    .replace(/\\\\/g, '\n')
  ...
  const paragraphs = plain
    ...
    .map(part => part.trim())
    .filter(Boolean)
    .map(part => `<p class="paragraph">${part.replace(/\n/g, '<br>')}</p>`)
    .join('\n');
...
<body>
  <h1>Cover Letter</h1>
  <div class="content">
    ${paragraphs || '<p class="paragraph">To,</p><p class="paragraph">Hiring Manager</p><p class="paragraph"><strong>Subject: Application for Target Role</strong></p><p class="paragraph">Dear Hiring Manager,</p><p class="paragraph">Please find attached my application for the target role.</p><p class="paragraph">Sincerely,<br>Candidate</p>'}
  </div>
</body>
</html>`;
}
