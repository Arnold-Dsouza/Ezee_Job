...
5. LATEX ESCAPING: Ensure all LaTeX special characters in textual fields are properly escaped:
   ...
   - Escape _ as \_
   - Escape { as \{ and } as \}
6. NO INDENT COMMAND RULE: Never use \noindent or \oindent in generated LaTeX. Use blank lines, \vspace, or normal paragraph flow instead.
7. COVER LETTER HEADING RULE: You MUST include "Cover Letter" as a bold centered heading (using \\begin{center}\\textbf{\\Large Cover Letter}\\end{center}) above the candidate contact header for all Cover Letter generations.
8. JSON OUTPUT ONLY: You MUST return a single valid JSON object.
...
function parseAiJsonResponse(text) {
  ...
  const fixMacroBackslashes = (tex) => {
    if (!tex) return '';
    let cleaned = tex.replace(/\\\\(documentclass|begin|end|section|subsection|subsubsection|textbf|textit|item|usepackage|pagestyle|geometry|href|url|vspace|hspace|small|large|Large|bfseries|itshape|raggedright|centering|par|definecolor|color|setlength|addtolength|renewenvironment|newcommand|author|title|date|maketitle|input|include|RequirePackage|ProcessOptions|LoadClass)/g, '\\$1');
    cleaned = cleaned.replace(/\\(?:noindent|oindent)\b/g, '');
