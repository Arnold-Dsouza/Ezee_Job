...
function parseAiJsonResponse(text) {
  ...
  const fixMacroBackslashes = (tex) => {
    ...
    // Repair malformed macro spacing such as "\\ oindent" -> "\\noindent"
    cleaned = cleaned.replace(/\\\s*oindent\b/gi, '\\noindent');

    // Repair common doubled-backslash macro escapes from JSON/string round-trips.
    cleaned = cleaned.replace(/\\(documentclass|begin|end|section|subsection|subsubsection|textbf|textit|item|usepackage|pagestyle|geometry|href|url|vspace|hspace|small|large|Large|bfseries|itshape|raggedright|centering|noindent|par|definecolor|color|setlength|addtolength|renewenvironment|newcommand|author|title|date|maketitle|input|include|RequirePackage|ProcessOptions|LoadClass)/g, '\\$1');
