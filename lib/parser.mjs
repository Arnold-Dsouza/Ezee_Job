...
export async function parsePdfToMarkdown(pdfBuffer) {
  try {
    ...
    const parseFn = pdfParseModule.default || pdfParseModule.pdfParse || pdfParseModule.PDFParse || pdfParseModule;

    if (typeof parseFn === 'function') {
      const data = await parseFn(pdfBuffer);
      rawText = data.text || '';
      pageCount = data.numpages || data.pages?.length || 1;
    } else if (typeof parseFn === 'object' && parseFn && typeof parseFn.getText === 'function') {
      const parser = new parseFn({ data: pdfBuffer });
